...
class ClassMerging:
    ...
    def __init__(self, parser, base_parsers: Dict, class_name: str):
        ...
        self.rewriter = Rewriter({'FunctionDef': self._merge_methods, 'Name': self._rewrite_name})
    ...
    def _parse_module(self, endpoints):
        ...
        try:
            ...
            with cd('.' * self.node.level):
                file_path = os.path.join(os.getcwd(), module_name.replace('.', os.sep) + '.py')
...
class AutoSlim:
    ...
    def generate(self):
        ...
        parser = Parser(entry)
        codegen = self._mode(self.target_dir, parser, class_merge_level=self._merge_class)
        codegen.generate()
        return self
